...
def user_view_case_status(request):
    ...
    c.execute("select * from case_request")
    data5=c.fetchall()
    print(data)
    if not bool(data):
        msgg = "No case Applications"
    # if 'login' in request.POST:
        # return HttpResponseRedirect("/login")
        return render(request,"user_view_case_status.html",{"data":data,"msgg":msgg,"data1":data1,"data2":data2,"data3":data3,"data5":data5})
    return render(request,"user_view_case_status.html",{"data":data,"data1":data1,"data2":data2,"data3":data3,"data5":data5})
...
def user_cat_list(request):
    s = "select * from category order by cat_name asc"
    # s = "SELECT *,BIN(ipc_section) AS binray_not_needed_column FROM ipc ORDER BY binray_not_needed_column ASC , ipc_section ASC"
    # s = "SELECT *  CAST(ipc_section as SIGNED) AS casted_column FROM ipc ORDER BY casted_column ASC , ipc_section ASC"
    print(s)
    c.execute(s)
