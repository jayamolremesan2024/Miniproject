...
def adv_view_rating(request):
    adv_id = request.session["adv_id"]
    print(adv_id)
    s = "select * from rating where adv_id = '"+str(adv_id)+"' "
    print(s)
    c.execute(s)
    conn.commit()
    data = c.fetchall()
    ...
    # if 'login' in request.POST:
        # return HttpResponseRedirect("/login")
    return render(request,"adv_rating.html",{"data":data})
...
def view_case_status(request):
    ...
    if "Stat" in request.POST:
        ...
        c.execute("insert into  casestatus (caseno,`date`,description) values(%s,%s,%s)",[cno,dat,desc])
        conn.commit()
    print(data)
    if not bool(data):
        msgg = "No case Applications"
...
def advocate_profile(request):
    ...
    print(data)
    if request.POST:
        quali=request.POST["t4"]
        c.execute("update  advocate set adv_qual='"+str(quali)+"' where adv_id='"+str(adv_id)+"'")
        conn.commit()
